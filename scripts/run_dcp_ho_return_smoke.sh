...
flow_file="$(mktemp "${REPO_ROOT}/config/dcp_ho_return_smoke_flow.XXXXXX.txt")"
history_existed=0
...
dcp_ho_rx_at_receiver="$(get_stat dcp_ho_rx_at_receiver)"
dcp_ho_returned="$(get_stat dcp_ho_returned)"
dcp_ho_rx_at_sender="$(get_stat dcp_ho_rx_at_sender)"

if [[ -z "${dcp_ho_rx_at_receiver}" || -z "${dcp_ho_returned}" || -z "${dcp_ho_rx_at_sender}" ]]; then
  echo "missing DCP HO return counters in ${stats_file}" >&2
  exit 1
fi
if (( dcp_ho_rx_at_receiver <= 0 )); then
  echo "expected dcp_ho_rx_at_receiver > 0, got ${dcp_ho_rx_at_receiver}" >&2
  exit 1
fi
if (( dcp_ho_returned <= 0 )); then
  echo "expected dcp_ho_returned > 0, got ${dcp_ho_returned}" >&2
  exit 1
fi
if (( dcp_ho_rx_at_sender <= 0 )); then
  echo "expected dcp_ho_rx_at_sender > 0, got ${dcp_ho_rx_at_sender}" >&2
  exit 1
fi

echo "dcp_ho_return_smoke=pass"
echo "run_id=${run_id}"
echo "stats_file=${stats_file}"
echo "dcp_ho_rx_at_receiver=${dcp_ho_rx_at_receiver}"
echo "dcp_ho_returned=${dcp_ho_returned}"
echo "dcp_ho_rx_at_sender=${dcp_ho_rx_at_sender}"
